    ...
    namespace Network
    {
        ...
        std::unique_ptr<Info> Server::ReceiveInfo(const std::type_info *infoType, int index)
        {
            ...
            static std::map<const std::type_info *, funcType> mapping{
                ...
                {&typeid(AllPlayersReadyInfo), [this](int index)
                 { return ReceiveInfoImpl<AllPlayersReadyInfo>(index); }},
                {&typeid(PayInfo), [this](int index)
                 { return ReceiveInfoImpl<PayInfo>(index); }},
        ...
        void Server::DeliverInfo(const std::type_info *infoType, int index, const Info &info)
        {
            ...
            static std::map<const std::type_info *, funcType> mapping{
                ...
                {&typeid(AllPlayersReadyInfo), [this](int index, const Info &info)
                 {
                     DeliverInfoImpl<AllPlayersReadyInfo>(index, dynamic_cast<const AllPlayersReadyInfo &>(info));
                 }},
                {&typeid(PayInfo), [this](int index, const Info &info)
                 {
                     DeliverInfoImpl<PayInfo>(index, dynamic_cast<const PayInfo &>(info));
                 }},
