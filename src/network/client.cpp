    ...
    namespace Network
    {
        ...
        std::unique_ptr<Info> Client::ReceiveInfo(const std::type_info *infoType)
        {
            ...
            static std::map<const std::type_info *, funcType> mapping{
                {&typeid(JoinGameInfo), [this]
                 { return ReceiveInfoImpl<JoinGameInfo>(); }},
                {&typeid(JoinGameRspInfo), [this]
                 { return ReceiveInfoImpl<JoinGameRspInfo>(); }},
                {&typeid(GameStartInfo), [this]
                 { return ReceiveInfoImpl<GameStartInfo>(); }},
                {&typeid(ActionInfo), [this]
                 { return ReceiveInfoImpl<ActionInfo>(); }},
                {&typeid(DrawInfo), [this]
                 { return ReceiveInfoImpl<DrawInfo>(); }},
                {&typeid(AllPlayersReadyInfo), [this]
                 { return ReceiveInfoImpl<AllPlayersReadyInfo>(); }},
                {&typeid(PayInfo), [this]
                 { return ReceiveInfoImpl<PayInfo>(); }},
                {&typeid(SkipInfo), [this]
                 { return ReceiveInfoImpl<SkipInfo>(); }},
                {&typeid(PlayInfo), [this]
                 { return ReceiveInfoImpl<PlayInfo>(); }},
                {&typeid(DrawRspInfo), [this]
                 { return ReceiveInfoImpl<DrawRspInfo>(); }}};
        ...
        void Client::DeliverInfo(const std::type_info *infoType, const Info &info)
        {
            ...
            static std::map<const std::type_info *, funcType> mapping{
                ...
                {&typeid(AllPlayersReadyInfo), [this](const Info &info)
                 {
                     DeliverInfoImpl<AllPlayersReadyInfo>(dynamic_cast<const AllPlayersReadyInfo &>(info));
                 }},
                {&typeid(PayInfo), [this](const Info &info)
                 {
                     DeliverInfoImpl<PayInfo>(dynamic_cast<const PayInfo &>(info));
                 }},
