...
namespace UNO
{
    namespace Game
    {
        ...
        void GameStartInfo::Serialize(uint8_t *buffer) const
        {
            ...
            msg->mLen = sizeof(Card) * 10 + sizeof(int) + usernames.size();
        ...
        {
            return (mInitHandCards == info.mInitHandCards) && (mFlippedCard == info.mFlippedCard) && (mFirstPlayer == info.mFirstPlayer) && (mUsernames == info.mUsernames);
        }
        ...
        {
            return (mNumber == info.mNumber) && (dynamic_cast<const ActionInfo &>(*this) == dynamic_cast<const ActionInfo &>(info));
        }
        ...
        {
            return (mCard == info.mCard) && (mNextColor == info.mNextColor) && (dynamic_cast<const ActionInfo &>(*this) == dynamic_cast<const ActionInfo &>(info));
        }
