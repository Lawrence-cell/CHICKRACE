    ...
    namespace Game
    {
        ...
        GameStat::GameStat(const GameStartInfo &info)
            : mCurrentPlayer(info.mFirstPlayer),
              mIsInClockwise(info.mFlippedCard.mText != CardText::TEN),
              mLastPlayedCard(info.mFlippedCard) {}

        GameStat::GameStat(int firstPlayer, Card flippedCard)
            : mCurrentPlayer(firstPlayer),
              mIsInClockwise(flippedCard.mText != CardText::TEN) {}

        void GameStat::NextPlayer()
        {
            mCurrentPlayer = mIsInClockwise ? Common::Util::WrapWithPlayerNum(mCurrentPlayer + 1) : Common::Util::WrapWithPlayerNum(mCurrentPlayer - 1);
            mTimeElapsed = 0;
        }
        ...
        void GameStat::UpdateAfterDraw()
        {
            ...
            mLastPlayedCard.mText = CardText::TEN;
        ...
        {
            if (mLastPlayedCard.mText == CardText::TEN)
            {
                // last played card will become EMPTY after the skip penalty is consumed
                mLastPlayedCard.mText = CardText::FIVE;
            }
            NextPlayer();
        }
